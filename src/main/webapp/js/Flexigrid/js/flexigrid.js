	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
            ...
            __mw: { //extendable middleware function holding object
                datacol: function(p, primaryId,colMode, col, val) { //middleware for formatting data columns
			//alert("datacol:"+col+":"+val)
                    var _col = (typeof p.datacol[col] == 'function') ? p.datacol[col](primaryId,colMode,col,val) : val; //format column using function
					...
					$.each(data.rows, function (i, row) {
						...
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								if (typeof row.cell == 'undefined' && p.datacol.length == 0) {
									td.innerHTML = g.getColModelValue(row,p.colModel[idx].name);//row[p.colModel[idx].name];//LEO UPDAE 2014-02-12
									if(p.colModel[idx].key){
										_primaryId = td.innerHTML;
									};
								} else {
                                    ...
                                    if (typeof row.cell[idx] != "undefined") {
                                        ...
                                        iHTML = g.getColModelValue(row.cell,p.colModel[idx].name);//row.cell[p.colModel[idx].name];LEO UPDATE 2014-02-18
                                        if(p.colModel[idx].key){//LEO UPDATE 2014-04-16传递KEY主键
    										_primaryId = iHTML;
    									};
                                        if(p.colModel[idx].type == 'checkbox'){//LEO UPDATE 2014-02-19
                                        	iHTML = "<input type='checkbox' name='"+p.colModel[idx].name+"' value='"+iHTML+"'>";
                                        }
                                        if(p.colModel[idx].type == 'radio'){
                                        	iHTML = "<input type='radio' name='"+p.colModel[idx].name+"' value='"+iHTML+"'>";
                                        }
                                    }
                                    
                                    row["cell"]=null;
                                    //alert(p.colModel[idx].name+":"+iHTML);
//                                  td.innerHTML = p.__mw.datacol(p, $(this).attr('abbr'), iHTML); //use middleware datacol to format cols
                                    td.innerHTML = p.__mw.datacol(p, _primaryId,p.colModel[idx],$(this).attr('abbr'), iHTML); //use middleware datacol to format cols
								}
								if(td.innerHTML == 'null') td.innerHTML = '';//LEO UPDATE 2014-03-24 
				...
				this.addCellProp();
				this.addRowProp();
				this.addRowCelHandle();//row,cell的后处理函数LEO UPDATE 2014-02-27
				this.rePosDrag();
				tbody = null;
				data = null;
				i = null;
			...
			getColModelValue:function(row,name){//LEO UPDATE 2014-02-12
				var e = name.split("."); 
				//alert(e.length);
				if(e.length >1){
					//return row[e[0]][e[1]];
					return this.getColModelValue(row[e[0]],e[1]);
				} else {
					return row[name];
				}
			},
			...
			addRowProp: function () {
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					if($(this).is(".trSelected")){
						$(this).find(":checkbox").attr("checked","checked");//LEO UPDATE  2014-02-19
						$(this).find(":radio").attr("checked","checked");
					} else {
						$(this).find(":checkbox").removeAttr("checked");//LEO UPDATE  2014-02-19
						$(this).find(":radio").removeAttr("checked");
					}
					
					if (p.singleSelect && ! g.multisel) {
						$(this).siblings().removeClass('trSelected');
						$(this).siblings().find(":checkbox").removeAttr("checked");//LEO UPDATE  2014-02-19
				...
				$('tbody tr', g.bDiv).find(":checkbox").on("click",function(){//LEO UPDATE 2014-02-19
			...
			addRowCelHandle : function(){
				$('tbody tr', g.bDiv).each(function(i,tr){
					$(tr).children().each(function(j,td){
						var col = $(td).attr("abbr");
						var val = $(td).find(":first").html();
						//if($(val).is("input")) val = $(val).val();
						if(typeof p.dataEvent[col] == 'function'){
							p.dataEvent[col](tr,val);
						}
					});
				});
			},
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$.each(sitems,function(i,item){
					if(item.type == 'text'){
						searchHtml += item.display+"&nbsp;:&nbsp;<input type='text' size='15' id='q_"+item.name+"' name='"+item.name+"' class='qsbox' />&nbsp;"
					} else if(item.type == 'checkbox'){
						searchHtml += item.display+"&nbsp;:&nbsp;<input type='checkbox' size='15' id='q_"+item.name+"' name='"+item.name+"' class='qsbox' />&nbsp;"
					}/* else if(item.type == 'radio'){
						searchHtml += item.display+"&nbsp;:&nbsp;";
						$.each(item.option,function(key,value){
							searchHtml += "<input type='radio' id='q_"+item.name+"_"+key+"' name='"+item.name+"' class='qsbox' value='"+key+"'>"+value+"&nbsp;";
						});
					}*/ else if(item.type == 'select'){
						searchHtml += item.display+"&nbsp;:&nbsp;<select id='q_"+item.name+"' name='"+item.name+"' style='width:15'>" ;
						$.each(item.option,function(key,value){
							searchHtml += "<option value='"+key+"'>"+value+"</option>";
						});
						searchHtml += "</select>&nbsp;";
					} else if(item.type == 'date'){
						_isdate = true;
						searchHtml += item.display+"&nbsp;:&nbsp;<input id='q_d_"+item.name+"' name='"+item.name+"' size='15' class='qsbox'/>&nbsp;" ;
					}
					if((i+1)%p.searchGroupLen == 0 || i==sitems.length-1){
						$(g.sDiv).append("<div class='sDiv2'>" + searchHtml + "</div><br/>");
						searchHtml = "";
					}
					//alert(searchHtml);
				});
				$(g.sDiv).append("<div class='sDiv2'>" +
						"<input name='q_search_submit' type='button' value='查询'>" +
						"<input name='q_search_reset' type='button' value='重置'></div>");
				
				//LEO UPDATE 2014-02-20
				$('input[name=q_search_submit]', g.sDiv).click(function (e) {
					g.doSearch();
				});
				$('input[name=q_search_reset]', g.sDiv).click(function (e) {
					$("[id^=q]").val("");
				});
				
				$(function(){//LEO UPDATE 2014-02-20
					try{
						if(_isdate){//have search item type is date
							$("[id^=q_d_]").datepicker( $.datepicker.regional[ "zh-CN" ] );
						}
					}catch(ex){
						alert("xxx"+ex.message);
					}
				})
	...
	$.fn.selectedRows = function (p) { // Returns the selected rows as an array, taken and adapted from http://stackoverflow.com/questions/11868404/flexigrid-get-selected-row-columns-values
		...
		$(selector).each(function (i, row) {
			...
			$.each(row.cells, function (c, cell) {
				...
				try{
					if($(val).is("input")) val= $(val).val();//LEO UPDATE 2014-02-19
				}catch(e){
					//alert(e.message);
				}
	...
	/*结果：{fieldName:fieldValue},如{oid:'1',name:'2'}*/
	$.fn.selectedSimpleRows = function (p) { // LEO UPDATE 2014-02-26
		var arReturn = [];
		var arRow = {};
		var selector = $(this.selector + ' .trSelected');
	...
	/*结果：{fieldName:fieldValue},如{oid:'1',name:'2'}*/
	$.fn.notSelectedSimpleRows = function (p) { // LEO UPDATE 2014-02-26
		var arReturn = [];
		var arRow = {};
		var selector = $(this.selector + ' tr:not(.trSelected)');
