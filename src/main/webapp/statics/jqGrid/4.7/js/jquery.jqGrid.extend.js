(function($){
	var DataGrid=function(options,$this){
		var dataGrid=typeof $this!="undefined"?$this:options.dataGrid?options.dataGrid:$("#dataGrid"),dataGridId=dataGrid.attr("id"),
		...
		options=$.extend(
				{
					...
					postData:searchForm.serializeArray(),mtype:"POST",
					datatype:"json",
					jsonReader:{id:options.dataId,root:"list",page:"pageNo",userdata:"otherData",total:"last",records:"count",subgrid:{root:"list"}},
					treeReader:{level_field:"treeLevel",parent_id_field:"parentCode",userdata:"otherData",leaf_field:"isTreeLeaf",expanded_field:"isOpen",icon_field:"_icon"},
					prmNames:{page:"pageNo",rows:"pageSize",sort:"orderBy",order:"sord",search:"_search",nd:"nd",id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del",subgridid:"id",npage:null,totalrows:"totalPage"},
					...
					loadComplete:function(data){
						...
						if(options.treeGrid){
							if(dataGrid.expandNodeIds){
								setTimeout(function(){
									if(dataGrid.expandNodeIds.length>0){$("#"+dataGrid.expandNodeIds.shift()+":visible .tree-plus",dataGrid).click()
										}else{if(dataGrid.currentLevel<dataGrid.expandLevel){dataGrid.currentLevel++;dataGrid.expandNodeIds=[];$(".jqgrow:visible .tree-plus",dataGrid).each(function(){var id=$(this).parents(".jqgrow").attr("id");dataGrid.expandNodeIds.push(id)});$("#"+dataGrid.expandNodeIds.shift()+":visible .tree-plus",dataGrid).click()}else{dataGrid.expandNodeIds=null}}},10)}else{if(options.defaultExpandLevel&&options.defaultExpandLevel>0){expandTreeNode(options.defaultExpandLevel);options.defaultExpandLevel=0}}setParam({postData:{id:"",nodeid:""}})}if(options.editGrid){$(function(){var ids=getDataIDs();if(ids.length>0){if(options.editGridInitAllRowEdit==true){for(var i=0;i<ids.length;i++){dataGrid.jqGrid("editRow",ids[i],{keys:true,focusField:false})}}}else{if(options.editGridInitRowNum&&options.editGridAddRow){for(var i=0;i<options.editGridInitRowNum;i++){options.editGridAddRow("init")}}}})}if(typeof options.ajaxSuccess=="function"){options.ajaxSuccess(data)}
							...
							if(typeof options.btnMoreEventBind=="function"){options.btnMoreEventBind(dataGrid.find(".btnMore"));options.btnMoreEventBind($("#"+dataGridId+"_frozen").find(".btnList"))}if(data&&data.message){js.showMessage(data.message)}resizeDataGrid();$(".btn").attr("disabled",false);js.closeLoading()},loadError:function(data){if(typeof options.ajaxError=="function"){options.ajaxError(data)}$(".btn").attr("disabled",false);js.showErrorMessage(data.responseText);js.closeLoading(0,true)},gridComplete:function(){if(typeof options.complete=="function"){options.complete()}resizeDataGrid()},onSortCol:function(index,iCol,sortorder){if(options.inputOrderBy&&options.inputOrderBy.length){options.inputOrderBy.val(index+" "+sortorder)}},
								...
								elements.each(function(){
									if($(this).attr("data-click-binded")==undefined){
										...
										$(this).click(function(){
											...
											if(title==undefined){title=se.attr("title")}
												...
												js.confirm(confirm,url,function(data){
													...
													if(data.result==Global.TRUE){
														...
														if(confirmSuccess!=undefined){
															try{eval(confirmSuccess)}catch(e){log("confirmSuccess error: "+e)}
														}else{
															if(options.treeGrid){
															...
															if(delTreeNode!=undefined){
																...
																if(row&&!isRoot(row.parentCode)){refreshTreeChildren(row.parentCode)}else{refreshTree()}
															}
															}else{refresh()}
														}
													}
													...
													console.log("options.tabPageId============>",options.tabPageId);
									...
									elements.each(function(){
										if($(this).attr("data-mouseover-binded")==undefined){
											...
											elements.mouseover(function(){
												var bdiv=dataGrid.closest(".ui-jqgrid-bdiv"),more=$(this),items=more.next(),pos=more.position(),top=pos.top-items.height()/2+6,left=pos.left-items.width()-2;
												if(!items.hasClass("moreItems")){return}
												dataGrid.find(".btnMore.open").removeClass("open").find("i").removeClass("fa-chevron-circle-left").addClass("fa-chevron-circle-right");dataGrid.find(".moreItems.open").removeClass("open").hide();
												var itemsTop=bdiv.scrollTop()+5;
												if(top<=itemsTop){top=itemsTop}else{
													itemsTop=bdiv.height()-items.height()+itemsTop-12;if(top>=itemsTop){top=itemsTop}
												}
												...
												if(items.attr("data-hover-binded")==undefined){
													...
													items.hover(function(){window.clearTimeout(timeoutHover)},
													function(){
														window.clearTimeout(timeoutHover);
														timeoutHover=window.setTimeout(function(){more.removeClass("open").find("i").removeClass("fa-chevron-circle-left").addClass("fa-chevron-circle-right");items.hide()},500)
													}).find("a").click(
														function(){
															more.removeClass("open").find("i").removeClass("fa-chevron-circle-left").addClass("fa-chevron-circle-right");items.hide()
														})}
												})
										}
									});return self},
				...
				if(options.columnModel.length>0){options.colNames=[];options.colModel=[];for(var i=0,model;i<options.columnModel.length;i++){model=options.columnModel[i];if(model.header){options.colNames.push(model.header)}else{if(model.label){options.colNames.push(model.label)}}options.colModel.push(model)}}
				if(options.treeGrid||!options.sortableColumn){for(var i=0;i<options.colModel.length;i++){options.colModel[i].sortable=false}}if(options.treeGrid){options.showRownum=false;options.showCheckbox=true}if(options.showRownum){options.rownumbers=true}if(options.showCheckbox){options.multiselect=true}if(options.datatype=="local"&&options.rowNum==-1){options.rowNum=5000}if(options.lazyLoad&&!options.treeGrid){options.datatype_bak=options.datatype;options.datatype="local"}if(options.groupHeaders||options.frozenCols||!options.shrinkToFit){options.shrinkToFit=false}else{options.shrinkToFit=true}if(options.showFooter){options.footerrow=true;options.userDataOnFooter=true}if(options.inputPageNo.length==0){var pageNo=searchForm.data("pageNo");searchForm.append('<input id="pageNo" name="pageNo" type="hidden" value="'+(pageNo?pageNo:"")+'"/>');options.inputPageNo=$("#pageNo",searchForm)}if(options.inputPageSize.length==0){var pageSize=searchForm.data("pageSize");searchForm.append('<input id="pageSize" name="pageSize" type="hidden" value="'+(pageSize?pageSize:"")+'"/>');options.inputPageSize=$("#pageSize",searchForm)}if(options.inputOrderBy.length==0){var orderBy=searchForm.data("orderBy");searchForm.append('<input id="orderBy" name="orderBy" type="hidden" value="'+(orderBy?orderBy:"")+'"/>');options.inputOrderBy=$("#orderBy",searchForm)}if(searchForm&&searchForm.length>0){searchForm.submit(function(){refresh();return false}).on("reset",function(){setTimeout(function(){if($.fn.iCheck!==undefined){searchForm.find("input[type=checkbox].form-control:not(.noicheck),input[type=radio].form-control:not(.noicheck)").iCheck("update")}if($.fn.select2!==undefined){searchForm.find("select.form-control:not(.noselect2)").trigger("change")}searchForm.find(".isReset").val("")},200)})}
				if(typeof options.btnEventBind=="function"){
					console.log("btnTool")
					options.btnEventBind($(".btnTool"))
				}
				if(options.btnSearch.length>0){options.btnSearch.unbind("click").click(function(){var btnSearch=$(this);if(searchForm.hasClass("hide")){searchForm.removeClass("hide");btnSearch.addClass("active");btnSearch.html(btnSearch.html().replace("查询","隐藏"))}else{searchForm.addClass("hide");btnSearch.removeClass("active");btnSearch.html(btnSearch.html().replace("隐藏","查询"))}resizeDataGrid();return false});if(!searchForm.hasClass("hide")){var btnSearch=options.btnSearch;searchForm.removeClass("hide");btnSearch.addClass("active");btnSearch.html(btnSearch.html().replace("查询","隐藏"))}}if(options.treeGrid){options.btnRefreshTree.unbind("click").click(function(){searchForm.each(function(){this.reset()});refreshTree();return false});options.btnExpandTreeNode.unbind("click").click(function(){expandTreeNode(1);return false});options.btnCollapseTreeNode.unbind("click").click(function(){collapseTreeNode();return false})}if(options.editGrid){options.editGridAddRowBtn.unbind("click").click(function(){if(options.editGridAddRow){options.editGridAddRow("new")}return false});if(options.editGridInputForm&&options.editGridInputForm.length>0&&options.editGridInputFormListName!=""&&options.editGridInputFormListAttrs!=""){options.editGridInputForm.submit(function(){updateListFieldName(options.editGridInputFormListName,options.editGridInputFormListAttrs)})}}dataGrid.jqGrid(options);if(options.groupHeaders&&options.groupHeaders.twoLevel){if(options.groupHeaders.threeLevel){dataGrid.jqGrid("setComplexHeaders",{complexHeaders:{defaultStyle:true,twoLevel:options.groupHeaders.twoLevel,threeLevel:options.groupHeaders.threeLevel}})}else{dataGrid.jqGrid("setGroupHeaders",{useColSpanStyle:true,groupHeaders:options.groupHeaders.twoLevel})}}if(options.frozenCols){dataGrid.jqGrid("setFrozenColumns")}$(window).resizeEnd(function(){resizeDataGrid()});resizeDataGrid();function setGridHeight(){if(!options.autoGridHeight){return}var gridHeight=0;if(typeof options.autoGridHeight=="function"){gridHeight=options.autoGridHeight()}else{var tabContent=$(dataGrid).parents(".tab-content");if(tabContent.length>0){var portlet=$(dataGrid).parents(".box.tabbable.autoHeight:not(.default)");if(portlet.length>0){var headerHeight=$(dataGrid).parents(".ui-jqgrid").find(".ui-jqgrid-hdiv").height();if(headerHeight==0){headerHeight=43;if(options.groupHeaders&&options.groupHeaders.twoLevel){if(options.groupHeaders.threeLevel){headerHeight*=3}else{headerHeight*=2}}}gridHeight=tabContent.height()-headerHeight-options.autoGridHeightFix}else{gridHeight=300}}else{var gridHeight=$(dataGrid).height();var gridParent=$(dataGrid).parent();if(gridParent.length!=0){gridHeight=gridParent.height()}gridHeight=($(window).height()-$("body").height()+gridHeight-options.autoGridHeightFix);if(gridHeight<150){gridHeight=150}gridParent.height(gridHeight)}}if(gridHeight!=0){dataGrid.jqGrid("setGridHeight",gridHeight)}}function setGridWidth(){if(!options.autoGridWidth){return}var gridWidth=0;if(typeof options.autoGridWidth=="function"){gridWidth=options.autoGridWidth()}else{var jqGridParent=$(dataGrid).parents(".ui-jqgrid").parent();if(jqGridParent.is(":visible")){gridWidth=jqGridParent.width()-2}}if(gridWidth!=0){dataGrid.jqGrid("setGridWidth",gridWidth-options.autoGridWidthFix,(options.shrinkToFit&&$(window).width()>500))}}function resizeDataGrid(){setGridHeight();setGridWidth();setTimeout(function(){setGridHeight();setGridWidth()},(!!navigator.userAgent.match(/MSIE 8.0/))?200:100)}function isRoot(code){return !code||code==""||code=="0"}function getParam(paramName){return dataGrid.jqGrid("getGridParam",paramName)}function setParam(params,overwrite){dataGrid.jqGrid("setGridParam",params,overwrite);return self}function getDataIDs(){return dataGrid.jqGrid("getDataIDs")}
				function getRowData(id){return dataGrid.jqGrid("getRowData",id)}
				function getSelectRow(){return getParam("selrow")}
				function getSelectRows(){return getParam("selarrrow")}
				function setSelectRow(id,isCancel){if(id==undefined&&isCancel==undefined){dataGrid.jqGrid("resetSelection")}else{if(id!=undefined&&isCancel){dataGrid.jqGrid("resetSelection",id)}else{dataGrid.jqGrid("setSelection",id)}}return self}
				function refresh(pageNo,pageSize){
					if(pageNo){options.inputPageNo.val(pageNo)}
					if(pageSize){options.inputPageSize.val(pageSize)}
					var params={};
					if(options.inputPageNo.val()!=""){params.page=options.inputPageNo.val()}
					if(options.inputPageSize.val()!=""){params.rowNum=options.inputPageSize.val()}
					if(options.lazyLoad){options.datatype=params.datatype=options.datatype_bak}
					setParam(params);
					reloadGrid();
					return self
				}
				function reloadGrid(){dataGrid.trigger("reloadGrid")}
				function delRowData(id){dataGrid.jqGrid("delRowData",id);return self}
				function delTreeNode(id){dataGrid.jqGrid("delTreeNode",id);return self}
				function expandTreeNode(level){
					js.loading();
					setTimeout(function(){dataGrid.expandLevel=level;dataGrid.currentLevel=1;dataGrid.expandNodeIds=[];
					$(".jqgrow:visible .tree-plus",dataGrid).each(function(){var id=$(this).parents(".jqgrow")[0].id;dataGrid.expandNodeIds.push(id)});
					if(dataGrid.expandNodeIds.length>100){js.showMessage("本次需要展开“"+dataGrid.expandNodeIds.length+"”个节点，因为展开节点过多，执行取消。");
					dataGrid.expandNodeIds=[]}else{$("#"+dataGrid.expandNodeIds.shift()+":visible .tree-plus",dataGrid).click()}
					for(var i=0;i<dataGrid.expandLevel;i++){$(".jqgrow:visible .tree-plus",dataGrid).click()}js.closeLoading()},10);return self}
				function collapseTreeNode(){js.loading();setTimeout(function(){$(".tree-minus",dataGrid).click();js.closeLoading()},10);return self}
				function refreshTree(expandLevel,parentCode){if(expandLevel){options.defaultExpandLevel=expandLevel}if(!isRoot(parentCode)){setParam({postData:{id:parentCode}})}else{if(parentCode!=undefined){options.defaultExpandLevel=options.initExpandLevel}}reloadGrid();return self}
				function refreshTreeChildren(rowid,currentRowid){if(currentRowid){delRowData(currentRowid)}if(isRoot(rowid)){reloadGrid(0,rowid)}else{if(rowid!=""){var treeclick=dataGrid.find("#"+rowid+" .treeclick");if(treeclick.hasClass("tree-leaf")){treeclick.addClass("tree-parent")}treeclick.addClass("tree-reload").click();if(treeclick.hasClass("tree-plus")){treeclick.click()}}}}
				function mergeCell(cellNames){var trs=$("#"+dataGridId+">tbody>tr:gt(0)");$.each(cellNames.split(","),function(idx,name){var bg=trs.eq(0).children("[aria-describedby='"+dataGridId+"_"+name+"']"),index=bg.index(),rowsp=1;trs.slice(1).each(function(ind2,tr){var me=$(tr).children("td").eq(index);if(bg.text()===me.text()){rowsp++;me.hide()}else{bg.attr("rowspan",rowsp);bg=me;rowsp=1}bg.attr("rowspan",rowsp)})})}
				function updateListFieldName(listName,fieldNames){$.each(fieldNames.split(","),function(idx,fieldName){if(fieldName!=""){var inputs=['[name="'+fieldName+'"]:not(div,a,script)','[name^="'+listName+'["][name$="[].'+fieldName+'"]:not(div,a,script)'];dataGrid.find(inputs.join(",")).each(function(idx,val){$(this).attr("name",listName+"["+idx+"]."+fieldName)});var defaultInputs=['[name="!'+fieldName+'"]:not(div,a,script)','[name^="!'+listName+'["][name$="[].'+fieldName+'"]:not(div,a,script)'];dataGrid.find(defaultInputs.join(",")).each(function(idx,val){$(this).attr("name","!"+listName+"["+idx+"]."+fieldName)})}})}
				var self={
						jqGrid:function(option,value,v2,v3,v4,v5){return dataGrid.jqGrid(option,value,v2,v3,v4,v5)},
						setParam:function(params,overwrite){return setParam(params,overwrite)},
						getParam:function(paramName){return getParam(paramName)},
						getDataIDs:function(){return getDataIDs()},
						getRowData:function(rowId){return getRowData(rowId)},
						getSelectRow:function(){return getSelectRow()},getSelectRows:function(){return getSelectRows()},
						setSelectRow:function(id,isCancel){return setSelectRow(id,isCancel)},
						refresh:function(pageNo,pageSize){return refresh(pageNo,pageSize)},
						reloadGrid:function(){return reloadGrid()},
						delRowData:function(id){return delRowData(id)},
						delTreeNode:function(id){return delTreeNode(id)},
						expandTreeNode:function(level){return expandTreeNode(level)},
						collapseTreeNode:function(){collapseTreeNode()},
						refreshTree:function(expandLevel,parentCode){return refreshTree(expandLevel,parentCode)},
						refreshTreeChildren:function(rowid,currentRowid){return refreshTreeChildren(rowid,currentRowid)},
						mergeCell:function(cellNames){return mergeCell(cellNames)},
						updateListFieldName:function(listName,fieldNames){updateListFieldName(listName,fieldNames)}
				};
				return self
			};
				$.fn.dataGrid=function(option,value,v2,v3,v4,v5){
					...
					if(method_call===undefined){
						...
						if(fn){var args=$.makeArray(arguments).slice(1);method_call=fn.apply(this,args)}
					}
					return(method_call===undefined)?$set:method_call
				};
				$.extend($.jgrid,{stripHtml:function(v){v=String(v);return v.replace(/<[^>]*>/g,"")}});
})(jQuery);
	function page(b,a,c){
		try{
			if(c&&c!=""){
				$("#"+c).dataGrid("refresh",b,a)
					...
					$(".ui-jqgrid-btable:eq(0)").dataGrid("refresh",b,a)
							...
							function pageHtml(c){
								if(!c.funcName){
									c.funcName="page"}
								if(!c.funcParam){c.funcParam=""}
								if(!c.pageNo){c.pageNo=1}
								if(!c.pageSize){c.pageSize=30}
								if(!c.first){c.first=1}
								if(!c.last){c.last=1}
								if(!c.prev){c.prev=1}
								if(!c.next){c.next=1}
								if(!c.count){c.count=0}
								if(!c.centerNum){c.centerNum=5}
								if(!c.pageInfo){c.pageInfo=""}
								c.bothNum=1;c.centerBegin=c.pageNo-parseInt(c.centerNum/2);
								if(c.centerBegin<c.first){c.centerBegin=c.first}c.centerEnd=c.centerBegin+c.centerNum-1;
								if(c.centerEnd>=c.last){c.centerEnd=c.last;c.centerBegin=c.centerEnd-c.centerNum+1}
								if(c.centerBegin==c.first){c.centerEnd=c.centerEnd+c.bothNum}
								if(c.centerEnd==c.last){c.centerBegin=c.centerBegin-c.bothNum}
								if(!(c.last-c.centerEnd>c.bothNum)){c.centerBegin--}
								if(c.centerBegin<c.first){c.centerBegin=c.first}var b=['<ul class="pagination">\n'],a=0;
								if(c.pageNo==c.first){
